...
const deleteABlog = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Unabled to delete | Please try again" });
  }
};
...
const updateABlog = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Something wen wrong updating ! Please try again" });
  }

  if (!currentBlogToUpdate) {
    return res.status(500).json({ message: "Update to update" });
  }
